...
export function SocialBlock({
  ...
  const config = (block.config || {}) as { links?: SocialLink[]; alignment?: "left" | "center" | "right" };
  ...
  if (isEditor) {
    return (
      <BlockWrapper blockId={block.id} animation={block.animation} isEditor>
        <div className="rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 py-6 px-4 text-center">
          <p className="text-slate-600 font-medium">Social links</p>
          <p className="text-sm text-slate-500 mt-1">
            {links.length === 0
              ? "Add your profile links in the settings"
        ...
        {links.map((link) => {
          ...
          return (
            <a
              ...
              className="flex h-11 w-11 items-center justify-center rounded-full bg-slate-200 text-slate-700 hover:bg-slate-300 hover:text-slate-900 transition-colors"
